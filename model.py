...
pretraind_model_name = ['GoogleNet', "ShuffleNet", "DenseNet", "SqueezeNet", 'EfficientNet', "ResNet18", "Resnext50", "Ensemble"]
...
def build_model():
    ...
    if args.model == 'GoogleNet':
        ...
        # model = timm.create_model('resnest101e',pretrained=True,num_classes=10)
        ...
        ## 모델을 해당 파일에서 선언해주기 위해
        model.to(device)
        model = torch.nn.DataParallel(model)
    elif args.model == 'ResNet18':
        model = torchvision.models.resnet18(pretrained=True)
        in_feat_num = model.fc.in_features
        model.fc = torch.nn.Linear(in_feat_num, out_features=num_classes, bias=True)
